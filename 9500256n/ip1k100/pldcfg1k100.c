...
#include "ip1k100.h"
#include "./utility/9000453a.h"	/* contains IP1K100[] data array See HFileGenerator.c */
...
{+D}
        ...
        ABSTRACT:       This module is used to configure the PLD by loading
                        data in IP1K100[] over the IP bus.

        CALLING
            SEQUENCE:   status = int PLDConfig1k100( struct cblk1k100 *c_blk);
                        Where:
...
int PLDConfig1k100( struct cblk1k100 *c_blk)
...
*/
 ...
 for( i = 0; i < length; i++)
 {
    ...
    if( (input_word(c_blk->nHandle, (word*)&c_blk->brd_ptr->control) & 0xF) != 0x03)   /* != 3 indicates configuration is done */
       break;
